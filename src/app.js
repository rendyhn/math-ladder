...
const UI_EN = {
  brandTag: 'Elementary → University',
  ...
  reviewBlurb: 'One worksheet drawing questions from all ⟦0⟧ topics of this level — good for revision before a test.',
  heroEyebrow: 'Mathematics · Elementary to University',
  heroTitle: 'From counting to eigenvalues, one rung at a time.',
  heroLede: '⟦0⟧ topics across four levels. Each has a lesson and a practice worksheet whose numbers are generated fresh every time you open it — with a fold-out answer key and print-ready pages.',
  ...
  date: 'Date',
  score: 'Score',
  fillNote: 'Fill-in answers accept whole numbers, decimals, fractions like <code>3/4</code> and expressions like <code>2sqrt(3)</code> or <code>5pi</code>. Separate several answers with commas.',
...
const CREDIT = 'Math Ladder · @rendyhn 2026';
...
LEVELS.forEach((lv, li) => {
  lv.index = li;
  lv.topics.forEach((t, ti) => { t.level = lv; t.index = ti; t.pool = t.gens.map(fn => ({ fn, topic: t.id })); TOPICS.set(t.id, t); });
  lv.review = { id: 'review-' + lv.id, review: true, level: lv, index: lv.topics.length, pool: lv.topics.flatMap(t => t.pool) };
...
function cleanInput(raw) {
  let s = String(raw).trim().toLowerCase().replace(/[−–—]/g, '-').replace(/[×·]/g, '*').replace(/÷/g, '/').replace(/π/g, 'pi').replace(/√/g, 'sqrt').replace(/²/g, '^2').replace(/³/g, '^3').replace(/[\u00A0\u202F]/g, ' ');
...
function decs(x) { const s = String(Math.round(Math.abs(x) * 1e8) / 1e8); const k = s.indexOf('.'); return k < 0 ? 0 : s.length - k - 1; }
function close(x, v, tol, typed) {
...
function checkFill(ck, input) {
  ...
  if (Array.isArray(v)) {
    ...
    if (ck.ord) return v.every((y, k) => close(nums[k], y, ck.tol, parts[k]));
    const used = v.map(() => false);
    return nums.every((x, k) => { const j = v.findIndex((y, jj) => !used[jj] && close(x, y, ck.tol, parts[k])); if (j < 0) return false; used[j] = true; return true; });
  }
  const x = parseNum(s); return x === null ? false : close(x, v, ck.tol, s);
}
...
function autoWrong(a) {
  const d = decs(a), step = d ? 10 ** -d : Math.abs(a) >= 100 ? pick([1, 10]) : 1;
  return shuffle([a + step, a - step, a + 2 * step, a - 2 * step, a * 2, a + 10 * step, a - 10 * step, -a, a + 3 * step].map(x => Math.round(x * 1e8) / 1e8));
}
...
function finalize(raw, type, topic) {
  ...
  if (it.type === 'fill') it.check = { v: raw.v !== undefined ? raw.v : typeof raw.a === 'number' ? raw.a : undefined, alt: raw.alt, tol: raw.tol, ord: raw.ord };
  return it;
}
...
function renderNav() {
  nav.innerHTML = LEVELS.map(lv => `
    <section class="nav-level" style="--lv: var(--${lv.color})" data-level="${lv.id}">
      <h2 class="nav-level-title"><span class="rung" aria-hidden="true">${lv.index + 1}</span><span>${esc(lvName(lv))}<small>${esc(lvBand(lv))}</small></span></h2>
      <ol class="nav-topics">
        ${lv.topics.map((t, i) => `<li data-search="${esc([t.title, t.blurb, tTitle(t), tBlurb(t)].join(' ').toLowerCase())}"><a href="#${t.id}" data-id="${t.id}"><span class="num">${String(i + 1).padStart(2, '0')}</span><span>${esc(tTitle(t))}</span></a></li>`).join('')}
        <li data-search="${esc(('mixed review ' + ui('mixedReview') + ' ' + lvShort(lv)).toLowerCase())}"><a href="#${lv.review.id}" data-id="${lv.review.id}" class="nav-review"><span class="num">★</span><span>${esc(ui('mixedReview'))}</span></a></li>
      </ol>
    </section>`).join('');
...
function renderHome() {
  ...
  <section class="hero">
    <div class="hero-copy">
      ...
      <div class="hero-cta"><a class="btn btn-primary" href="#${first.id}">${esc(ui('startWith', tTitle(first)))}</a><a class="btn" href="#${LEVELS[2].review.id}">${esc(ui('tryReview'))}</a></div>
    </div>
  ...
  <section class="ladder" aria-label="${esc(ui('levels'))}">
    ...
    <article class="level" style="--lv: var(--${lv.color})">
      <header class="level-head"><span class="rung rung-lg" aria-hidden="true">${lv.index + 1}</span><div><h2>${esc(lvName(lv))}</h2><p class="band">${esc(lvBand(lv))} · ${esc(ui('nTopics', lv.topics.length))}</p></div></header>
      <p class="level-blurb">${esc(lvBlurb(lv))}</p>
      <ol class="level-topics">${lv.topics.map(t => `<li><a href="#${t.id}">${esc(tTitle(t))}</a></li>`).join('')}</ol>
      <a class="level-review" href="#${lv.review.id}">${esc(ui('reviewLink'))}</a>
    </article>`).join('')}
  </section>`;
  renderSample();
}
...
function renderTopic(t, tab) {
  const lv = t.level, pos = t.review ? ui('mixedReview') : ui('topicPos', t.index + 1, lv.topics.length);
  document.title = `${tTitle(t)} · Math Ladder`;
  main.innerHTML = `
  <article class="topic" style="--lv: var(--${lv.color})">
    <header class="topic-head">
      ...
      <p class="eyebrow"><span class="rung" aria-hidden="true">${lv.index + 1}</span>${esc(lvBand(lv))} · ${esc(pos)}</p>
      <h1>${esc(tTitle(t))}</h1>
